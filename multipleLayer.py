...
with graph.as_default():
    ...
    logits_final = tf.matmul(logits2, weights3) + biases3

    loss1 = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits_final, tf_train_labels))
    loss2 = beta*(tf.nn.l2_loss(weights3)+tf.nn.l2_loss(weights2)+tf.nn.l2_loss(weights1))
    ...
    logits_valid1 = tf.nn.relu(tf.matmul(tf_valid_dataset, weights1) + biases1)
    # logits_valid2 = tf.nn.dropout(logits_valid2, 0.5)
    logits_valid2 = tf.nn.relu(tf.matmul(logits_valid1, weights2)+biases2)
   ...
   # valid_prediction = tf.nn.softmax(tf.matmul(tf.nn.relu(tf.matmul(tf_valid_dataset,weights1)+biases1),weights2)+biases2)
    ...
    logits_test1 = tf.nn.relu(tf.matmul(tf_test_dataset, weights1) + biases1)
    # logits_test2 = tf.nn.dropout(logits_test2, 0.5)
    logits_test2 = tf.nn.relu(tf.matmul(logits_test1, weights2)+biases2)
...
with tf.Session(graph=graph) as session:
    ...
    for step in range(num_steps):
        ...
        _, l, logits, predictions, l1, l2 = session.run(
            [optimizer, loss, logits_final, train_prediction, loss1, loss2], feed_dict=feed_dict)
        if (step % 500 == 0):
